...
def X():
    return [
        [[1, 2, 0], [0, 0, 1], [1, 1, 2], [2, 2, 2], [3, 1, 0]],
        [[5, 1, 4], [2, 1, 0], [1, 0, 2], [1, 1, 0], [0, 2, 1]],
    ]
...
def test_reset_cache(model, X):
    ...
    assert_array_almost_equal(
        model._xw_sum,
        [[2.666838e-04, 1.895245e00], [2.635103e00, 3.469387e00]],
        4,
    )
...
def test_sample(model):
    ...
    assert_array_almost_equal(
        X[1],
        [
            [1.7823e00, 1.9666e-01, 1.5899e-01],
            [9.6352e-01, 9.5312e-02, 4.2323e-03],
            [2.9002e00, 1.2545e-01, 7.7268e-02],
            [9.4207e-01, 6.1731e00, 9.2237e-02],
            [8.8499e00, 6.5642e00, 3.1673e00],
            [4.4337e-01, 6.5174e-01, 9.6875e00],
        ],
        3,
    )
    assert_array_almost_equal(
        X[2],
        [
            [2.2685e00, 1.3926e00, 1.8356e00],
            [2.0275e00, 1.3866e-02, 1.9275e00],
            [1.2650e00, 1.5342e-01, 3.9146e-01],
            [1.3925e01, 2.7539e-01, 1.6151e-02],
            [7.5165e-01, 6.1712e00, 2.5927e-01],
            [3.4823e00, 4.6675e-01, 1.5978e-01],
        ],
        3,
    )
...
def test_forward_backward(model, X):
    ...
    assert_array_almost_equal(
        ...
        [
            [[2.6353e-04, 1.4304e-01], [8.8289e-01, 2.9738e00]],
            [[3.1500e-06, 1.7522e00], [1.7522e00, 4.9559e-01]],
        ],
        3,
    )
    ...
    assert_array_almost_equal(
        ...
        [
            [
                [-1.9921e00, -1.4665e-01],
                [-4.9764e00, -6.9228e-03],
                [-1.7232e01, -3.2831e-08],
                [-2.1118e01, -6.7415e-10],
                [-1.3209e-01, -2.0896e00],
            ],
            [
                [-3.7342e01, 0.0000e00],
                [-1.2056e-01, -2.1752e00],
                [-1.6357e01, -7.8752e-08],
                [-1.4412e-01, -2.0083e00],
                [-1.2218e01, -4.9402e-06],
            ],
        ],
        3,
    )

    assert_array_almost_equal(starts, [[1.3641e-01, 8.6359e-01], [6.0619e-17, 1.0000e00]], 3)

    assert_array_almost_equal(ends, [[8.7626e-01, 1.2374e-01], [4.9402e-06, 1.0000e00]], 3)
    ...
    assert_array_almost_equal(
        ...
        [
            [
                ...
                [3.2831e-08, 1.0000e00],
                [6.7415e-10, 1.0000e00],
                [8.7626e-01, 1.2374e-01],
            ],
            [
                [6.0619e-17, 1.0000e00],
                [8.8642e-01, 1.1358e-01],
                [7.8752e-08, 1.0000e00],
                [8.6578e-01, 1.3422e-01],
                [4.9402e-06, 1.0000e00],
            ],
        ],
        4,
    )
...
def test_partial_summarize(model, X):
    ...
    assert_array_almost_equal(
        model._xw_sum,
        [[2.635337e-04, 1.430405e-01], [8.828942e-01, 2.973798e00]],
        4,
    )
...
def test_summarize_weighted(model, X, w):
    ...
    assert_array_almost_equal(
        model._xw_sum,
        [[2.707788e-04, 4.173112e00], [4.912973e00, 4.113652e00]],
        4,
    )
    ...
    assert_array_almost_equal(d1._xw_sum, [8.834015e00, 5.179160e00, 6.910696e-03], 4)
...
def test_from_summaries_weighted(model, X, w):
    ...
    assert_array_almost_equal(d1.scales, [1.749434e00, 1.025649e00, 1.368553e-03])
    assert_array_almost_equal(d1._w_sum, [0.0, 0.0, 0.0])
    assert_array_almost_equal(d1._xw_sum, [0.0, 0.0, 0.0])
...
def test_from_summaries_inertia(X):
    ...
    d = [
        Exponential([2.1, 0.3, 0.1], inertia=0.25),
...
def test_from_summaries_weighted_inertia(X, w):
    ...
    assert_array_almost_equal(d1.scales, [1.749434e00, 1.025649e00, 1.368553e-03], 3)
    assert_array_almost_equal(d1._w_sum, [0.0, 0.0, 0.0])
    assert_array_almost_equal(d1._xw_sum, [0.0, 0.0, 0.0])
...
def test_from_summaries_frozen(model, X):
    ...
    d = [
        Exponential([2.1, 0.3, 0.1], frozen=True),
...
def test_fit(X):
    ...
    assert_array_almost_equal(model.starts, [-1.489857e01, -3.385568e-07], 4)
    ...
    assert_array_almost_equal(model.starts, [-1.545504e01, -1.940718e-07], 4)
...
def test_fit_weighted(X, w):
    ...
    assert_array_almost_equal(model.starts, [-1.5399e01, -2.0519e-07], 3)
    ...
    assert_array_almost_equal(model.starts, [-1.6093e01, -1.0250e-07], 3)
...
def test_masked_forward_backward(model, X, X_masked):
    ...
    assert_array_almost_equal(
        t,
        [
            [[0.0417, 0.6437], [1.4056, 1.9091]],
            [[0.0921, 1.6185], [1.7677, 0.5218]],
        ],
        3,
    )
    ...
    assert_array_almost_equal(starts, [[1.1432e-01, 8.8568e-01], [6.1237e-17, 1.0000e00]], 3)
    ...
    assert_array_almost_equal(
        ...
        [
            [
                ...
                [5.0183e-10, 1.0000e00],
                [8.7626e-01, 1.2374e-01],
            ],
            [
                [6.1237e-17, 1.0000e00],
                [8.8186e-01, 1.1814e-01],
                [8.3094e-09, 1.0000e00],
                [8.2871e-01, 1.7129e-01],
                [1.4918e-01, 8.5083e-01],
            ],
        ],
        4,
    )
...
def test_masked_summarize(model, X, X_masked, w):
    ...
    assert_array_almost_equal(d1._xw_sum, [4.6491e00, 5.0392e00, 3.9227e-08], 4)
...
def test_masked_from_summaries(model, X_masked, w):
    ...
    assert_array_almost_equal(d1.scales, [1.4869e00, 1.0232e00, 7.9650e-09], 4)
    assert_array_almost_equal(d1._w_sum, [0.0, 0.0, 0.0])
    assert_array_almost_equal(d1._xw_sum, [0.0, 0.0, 0.0])
