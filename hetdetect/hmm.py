...
def hmm_decode(infile, outfile, plot_file):
    ...
    with np.nditer(baf_full, op_flags=['readwrite']) as it:
        for x in it:
            if x > 0.5:
                x[...] = 1 - x
    BAF = torch.tensor(np.array(baf_full).reshape(-1, 39846, 1))
    ...
    trans_prob = [
        [1 - 4 / 30000, 1 / 30000, 1 / 30000, 1 / 30000, 1 / 30000],
    ...
    dists = [
        Normal([0.1], [[0.2]], covariance_type='diag'),
