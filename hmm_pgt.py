        ...
        with np.nditer(baf_full, op_flags=['readwrite']) as it:
            for x in it:
                if x > 0.5:
                    x[...] = 1 - x
        ## START TIMING HERE
        BAF = torch.tensor(np.array(baf_full).reshape(-1, baf_full.size, 1))
...
def main(infile, outfile, plot_file):
    # infile    =  pickle obj, list containing chromosomes' parsed snps (from filter_snps.py)
